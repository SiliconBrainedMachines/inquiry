...
- `pre_pr_inspection_report`: `cleanrooms/<branch>/pre_pr_inspection.md` on the closure path into END, auto-seeded on `finish_execute`, with an auto-generated `Consistency` pass refreshed at `pr_ready`, plus `Completeness` and `Traceability` passes, overall verdict, and `file:line` citations for `FAIL` findings
