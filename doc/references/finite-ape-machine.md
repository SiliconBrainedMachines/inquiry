...
An **ape** is a specialized AI sub-agent modeled as a Finite State Machine. Each ape has:

- **A prompt** that defines its transition function: δ(state, context) → new_state + output.
- **A set of skills** (tools it can invoke: memory, markitdown, mermaid, test runner, etc.).
- **A defined FSM** with explicit states, transitions, and termination conditions.
- **Clean context** on each instantiation — apes are reutilizable but stateless between invocations. They receive their task and the context they need; they can request additional context from the context provider.

Skills can be shared across apes (e.g., memory consultation, @contract reading, state reporting). Specialized skills define the ape's identity.
...
1. First consults **project memory** (cheap, local).
2. If insufficient, consults **framework memory** (potentially richer, broader scope).
3. The context provider serves relevant information without the ape needing to know where it came from.

Implementation: Engram or equivalent (SQLite + FTS5, MCP server, HTTP API).
