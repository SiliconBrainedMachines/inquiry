...
| Schema | Frontmatter YAML structure (enforced by SCRIBE) |
...
| Schema validation | SCRIBE skill (the "compiler") |
...
Every ape that reads memory receives this instruction as part of the SCRIBE skill:
...
Every file in `.ape/memory/` MUST begin with a YAML frontmatter block. This is the schema — non-negotiable, enforced by SCRIBE.
...
Each memory directory contains an `index.md` that serves as the primary index. TRACKER maintains these automatically.

```markdown
# ADR Index

> Auto-generated by TRACKER. Do not edit manually.
...
## 5. SCRIBE — The Documentation Compiler

### 5.1 What SCRIBE Is

SCRIBE is a **shared skill** available to every ape. It is not an agent — it is a set of strict rules embedded in each ape's prompt that governs how documentation is written, structured, and validated. SCRIBE is to Memory as Code what a compiler is to source code: it enforces syntax, validates structure, rejects malformed output, and ensures consistency.

### 5.2 What SCRIBE Enforces
...
### 5.3 SCRIBE Validation Checklist
...
### 5.4 SCRIBE as Prompt Injection

SCRIBE is implemented as a section in every ape's prompt:
...
### 5.5 Future: SCRIBE as Automated Validator

In v1.x.x, SCRIBE evolves from a prompt-based protocol to an automated validator:
...
3. Read REVIEWER reports for current cycle
...
4. Update indices (via SCRIBE protocol)
...
6. Validate via SCRIBE protocol
```
...
|--------|-------------|-----------------|
| SCOUT | `.ape/memory/` (ingested docs, but typically not memory) | indices |
| ANALYST | `specs/` | other ape's outputs, indices |
| ARCHITECT | `specs/` (WBS addendum) | other ape's outputs, indices |
| STRATEGIST | `runbooks/` | other ape's outputs, indices |
| TESTER | — (writes to source code, not memory) | memory files |
| CODER | `deviations/` | other ape's outputs, indices |
| REVIEWER | — (writes to PR comments, not memory) | memory files |
| DARWIN | `lessons/`, `reports/` | other ape's outputs |
| TRACKER | All `index.md` files, `status.md` | individual memory files |

**Rule:** Each ape writes only to its designated directories. Only TRACKER writes to shared files (indices). This eliminates write conflicts by design.
...
- SCRIBE is a prompt-based protocol.
...
- SCRIBE gets automated validation via `ape repo doctor --memory`.
...
│
└── status.md                           # Current state (TRACKER maintains)
```
...
| **SCRIBE** | Shared skill (documentation compiler) that enforces schema, structure, and cross-reference integrity on all memory files |
...
| **TRACKER** | Hook that maintains index files and project status, the only writer to shared files |
...
| **Schema violation** | Any memory file that fails SCRIBE validation — treated as a bug, not a style issue |
