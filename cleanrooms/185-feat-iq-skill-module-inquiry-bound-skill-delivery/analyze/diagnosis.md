...
6. The asset tree is already partially migrated. [code/cli/assets/instructions](../../../code/cli/assets/instructions) contains `doc-read.md`, `doc-write.md`, `issue-create.md`, `issue-end.md`, and `issue-start.md`, while [code/cli/assets/skills](../../../code/cli/assets/skills) still contains both Inquiry-private protocols and universal thinking tools. The migration is therefore incomplete and currently duplicated, but the transition-owned runtime slice is narrower than the full legacy private-skill set.
...
5. Treat the current mixed `skills/` directory as transitional state that must be untangled, not as the desired steady state.
