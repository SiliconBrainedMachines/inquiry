...
- [ ] **RED**: Escribir test que ejecuta `DoctorCommand` con un directorio home mock donde `~/.copilot/skills/` contiene 4 skills → assert `totalSkills == 4` y `missingSkills` vacío.
- [ ] **GREEN**: Investigar `doctor.dart` — localizar por qué el conteo de skills da 0. Hipótesis: la ruta de búsqueda no coincide con la ruta de deployment real. Corregir la lógica de escaneo.
- [ ] **REFACTOR**: Asegurar que el test cubre caso edge (0 skills = recién instalado, parcial = 2 de 4).
- [ ] Verificar manualmente: `iq doctor` reporta conteo correcto.
...
- [ ] **RED**: Escribir test que ejecuta `InitCommand` en tmpDir → assert que `cleanrooms/` se crea en tmpDir root, NO en `docs/cleanrooms`.
- [ ] **GREEN**: Investigar `init.dart` — localizar la construcción de ruta. Hipótesis: `p.join()` con path incorrecto o confusión docs/ vs root. Corregir.
- [ ] **REFACTOR**: Verificar idempotencia (ejecutar 2 veces no duplica).
- [ ] Verificar manualmente: `iq init` en repo limpio crea `./cleanrooms/`.
