...
Pass 3 now includes an automatic traceability check against the report metadata itself: `issue:` and `branch:` in `cleanrooms/<branch>/pre_pr_inspection.md` must match the active cycle, and any mismatch becomes a blocking `FAIL` with a repo-relative `file:line` citation. Keep adding manual issue/plan-mapping findings after the automatic traceability lines when they add real evidence.
